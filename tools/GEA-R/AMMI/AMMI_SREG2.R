...
for (b in 1:length(traits)){ 
  ...
  #newfold=paste("Output&",paste(tmp,paste(analysis,paste(ExpDes,paste("&",traits[b],sep=""),sep=""),sep=""),sep=""),sep="") ##inalis ko
  #sal=paste(orgdir,paste("/",newfold,sep=""),sep="") ## inalis ko uli
  #dir.create(sal) ##inalis ko pa din
  dir.create(tmp)
  #setwd(sal) #inalis ko pa uli
  setwd(tmp)
